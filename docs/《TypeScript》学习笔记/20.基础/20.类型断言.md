---
title: 类型断言
date: 2021-08-25 16:57:20
permalink: /pages/c742c7/
categories:
  - 《TypeScript》学习笔记
tags:
  - TypeScript
...
故建议大家在使用类型断言时，统一使用<mark>值 as 类型</mark>这样的语法，本书中也会贯彻这一思想。
...
swim(tom);
// Uncaught TypeError: animal.swim is not a function`is'.
...
本小节的前置知识点：<mark>结构类型系统（TODO）</mark>、<mark>类型兼容性（TODO）</mark>
...
需要注意的是，这里我们使用了简化的父类子类的关系来表达类型的兼容性，而实际上 TypeScript 在判断类型的兼容性时，比这种情况复杂很多，详细请参考类型的兼容性（TODO)章节。
